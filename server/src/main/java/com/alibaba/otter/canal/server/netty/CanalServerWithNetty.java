...
public class CanalServerWithNetty extends AbstractCanalLifeCycle implements CanalServer {
    ...
    public void start() {
        ...
        this.bootstrap = new ServerBootstrap(
                new NioServerSocketChannelFactory(new NioEventLoopGroup(1, new ThreadFactory() {
                    final private AtomicLong THREAD_COUNT = new AtomicLong(1);

                    @Override
                    public Thread newThread(Runnable r) {
                        Thread t = new Thread(r);
                        t.setName("Canal-stinson-thread-boss-" + THREAD_COUNT.getAndIncrement());
                        t.setDaemon(true);
                        return t;
                    }
                    ...
                    public Thread newThread(Runnable r) {
                        Thread t = new Thread(r);
                        t.setName("Canal-stinson-thread-worker-" + THREAD_COUNT.getAndIncrement());
                        t.setDaemon(true);
                        return t;
                    }
                }))
